...
createImageList <- function(fileNames, subjectIDs = NULL, imageIDs = NULL, ...){
  ...
  if (is.null(subjectIDs)) {
    warning("Blank subjectIDs field")
    for (i in 1 : length(rval)) rval[[i]]$subjectId <- NULL
  }
  else {
    ...
    if (anyDuplicated(subjectIDs) > 0) warning("Duplicate subjectIDs")
    for (i in 1 : length(rval)) rval[[i]]$subjectId <- subjectIDs[i]  
  }
  if (is.null(imageIDs)) {
    warning("Blank image ids")
    for (i in 1 : length(rval)) rval[[i]]$imageId <- NULL
  }
  else {
